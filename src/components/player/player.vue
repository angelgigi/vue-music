...
export default {
  ...
  methods: {
    ...
    _getPosAndScale() {
      const targetWidth = 40; //唱片缩略图高度
      const paddingLeft = 40;
      const paddingBottom = 30; //唱片缩略图距离底部的高度
      const paddingTop = 80; //顶部标题的高度
      const width = window.innerWidth * 0.8; //唱片宽度
      const scale = targetWidth / width; //缩放比例
      const x = -(window.innerWidth / 2 - paddingLeft); //向上的偏移值
